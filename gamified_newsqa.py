...
class GamifiedNewsQA(gym.Env):
    ...
    def load_dataset(self, split):
        ...
        file_path = self.data_path + "/" + split_map[split] + ".csv"
        with codecs.open(file_path, mode='r', encoding='utf-8', errors='ignore') as csv_file:
            ...
            for line in tqdm(csv_reader):
                if line_count == 0:
                    line_count += 1
                    continue
                _story, _question, _a_ranges = line[1], line[2], line[3]
    ...
    def reset(self, random=True):
        ...
        for idx in indices:
            ...
            tmp_story_sentence_list = []
            for i in range(len(story_sentence_list)):
                if len(story_sentence_list[i]) == 0:
                    continue
                if len(story_sentence_list[i].split()) <= 100:
                    tmp_story_sentence_list.append(story_sentence_list[i])
            # when none of the sentence contains any answer, ignore this datapoint
            if not self.answer_in_list_of_sentences(tmp_story_sentence_list, self.data["answers"][idx]):
                continue
            story_sentence_list = tmp_story_sentence_list
